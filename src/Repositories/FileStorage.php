...
class FileStorage
{
	...
	{
		$storageDir = $_SERVER["DOCUMENT_ROOT"] . "\data\chats";
		$this->nameStorage = $nameStorage;
		$this->dirPath = $storageDir . '\\' . $nameStorage;
	...
	public function post(array $data, bool $fromGet = false): array
	{
		...
		if ($this->nameStorage === "chats") {
			if (!array_key_exists('lot_id', $data)) return ['status' => false, 'message' => 'Field lot_id is required!'];
			if (!array_key_exists('executor_id', $data)) return ['status' => false, 'message' => 'Field executor is required!'];
